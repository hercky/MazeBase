...
class PBS2Type(RewardOnEndMixin, WithWaterBlocksTrapsMixin, BaseVocabulary):
    pass




class TriggerAgent(agents.SingleTileMovable):
    pass
...
def add_vertical_wall(game):
    """
    creates a vertical wall with an opening 
    for dividing the 
    """
...
class TrapKey(PBS2Type): 
    ...
    def _side_information(self):
        return super(TrapKey, self)._side_information() + \
            [[self.FEATURE.GOTO] + self.goal.featurize()]
...
class ChestKey(RewardOnEndMixin, WithWaterBlocksChestsMixin, BaseVocabulary): 
    ''' Agent must open a door with a switch and go to the goal '''
    ...
    def _side_information(self):
        return super(ChestKey, self)._side_information() + \
            [[self.FEATURE.GOTO] + self.goal.featurize()]
